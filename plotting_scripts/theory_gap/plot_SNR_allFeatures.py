...
def _apply_global_style():
    ...
    mpl.rcParams["axes.unicode_minus"] = False  # use ASCII '-' instead of U+2212
...
CACHE_CFG = {
    ...
    "cache_filename": "plot_SNR_allFeatures__cache.pkl",
    "verbose": True,
}
...
TEXT_CFG = {
    ...
    "x_tick_size": 5,
    "y_tick_size": 5,
    "x_tick_rotation": 0,           # degrees
}

FIG_CFG = {
    "figsize": (1.6, 1.45),  # inches (width, height)
    "dpi": 150,
    "tight_layout": True,
...
AXES_CFG = {
    ...
    "y_scale": "log",
    "y_lim": (1e-4, 1e-1),           # (ymin, ymax) or None
    "x_lim": None,

    "y_ticks": (1e-4, 1e-3, 1e-2, 1e-1),  # list of values, or None for auto
    "y_ticklabels": None,           # list of strings, or None => formatter
    "y_log_base": 10,
...
LEGEND_CFG = {
    "show": False,                  # only one series; can turn on if you want
    "label": r"$\mathrm{SNR}$",
    "loc": "upper right",
    "fontsize": 7,
    "frameon": False,
...
# ============================================================
# ========== CORE: INVERSE SNR PER MODEL / FEATURE ===========
# ============================================================

def compute_snr_for_model_and_feature(model_cfg: dict, feature: str):
    """
    For a given model entry in `paths` and a given feature, compute:
      - snr_avg
      - snr_std
    where SNR is taken from theory (finite-sample), using the same
    global/local/theory logic as in the original script, but per-feature.
    """
    ...
    if std_of_avg_split_and_file and multi_file_global:
        ...
        cv_global_mse_avg_std = mse_avg_std  # kept for completeness, unused
    ...
    if path_theory.is_dir() and any(p.is_file() and p.suffix.lower() == ".pkl" for p in path_theory.iterdir()):
        ...
        snr_avg = float(detailed_results["SNR"])
        snr_std = 0.0
        ...
        for i, run_dir in enumerate(run_dirs):
            ...
            snr_per_file.append(float(detailed_results["SNR"]))

        snr_avg = float(np.mean(snr_per_file))
        if std_of_avg_split_and_file:
            snr_std = float(np.std(snr_per_file) / np.sqrt(len(snr_per_file)))
        else:
            snr_std = float(np.std(snr_per_file))
    else:
        raise FileNotFoundError(f"Theory folder does not exist or isn’t a .pkl file/folder: {path_theory}")

    return snr_avg, snr_std
...
def main():
    ...
    cache_file = Path(CACHE_CFG.get("cache_filename", "plot_SNR_allFeatures__cache.pkl")).expanduser()
    ...
    if cache_enabled and (not force) and cache_file.exists():
        try:
            ...
            if isinstance(cached, dict) and cached.get("signature", {}).get("sig_hash") == sig["sig_hash"]:
                ...
                snr_means = cached["snr_means"]
                snr_stds = cached["snr_stds"]
            else:
                if CACHE_CFG.get("verbose", True):
    ...
    if snr_means is None or snr_stds is None:
        snr_means = np.zeros((n_features, n_models), dtype=float)
        snr_stds = np.zeros((n_features, n_models), dtype=float)

        for m_idx, (model_name, model_cfg) in enumerate(paths.items()):
            print(f"\nModel: {model_name}")
            for f_idx, feature in enumerate(FEATURES):
                print(f"  Feature: {feature}")
                avg_val, std_val = compute_snr_for_model_and_feature(model_cfg, feature)
                snr_means[f_idx, m_idx] = avg_val
                snr_stds[f_idx, m_idx] = std_val

        if cache_enabled:
            payload = dict(signature=sig, snr_means=snr_means, snr_stds=snr_stds)
            try:
    ...
    for m_idx, mid in enumerate(model_ids):
        ...
        for f_idx, feat_label in enumerate(FEATURE_TICK_LABELS):
            ...
            x_flat.append(x)
            vals_flat.append(snr_means[f_idx, m_idx])
            std_flat.append(snr_stds[f_idx, m_idx])
    ...
    # X ticks: one per (model, feature) bar; label = "Feature\nModel"
