...
for model_name in model_names:
    ...
    mean_centroid_norm_sq = float(np.mean(centroid_norms**2))
    mean_centroid_norm = float(np.mean(centroid_norms))
...
# =========================
# PLOTTING (CONFIGURABLE): ONLY Eigenvalues
# =========================
...
PLOT_CFG = {
    ...
    "dpi": 300,
    "save_path": "eigs_multi_model.svg",
    ...
    # ---- Titles & labels ----
    "show_suptitle": False,
    "suptitle": f"Across-category stats up to {int(X*100)}% variance — feature: {feature}",
    "suptitle_fontsize": 7,
    "suptitle_pad": 8.0,

    "xlabel": "Rank",
    "overlap_ylabel": "ov.", # unused here
    "eig_ylabel": "eigenvalues",
    "show_xlabel": True,
    "show_overlap_ylabel": True,  # unused here
    "show_eig_ylabel": True,
    "xlabel_fontsize": 7,
    "ylabel_fontsize": 7,
    ...
    "x_scale": "log",
    "x_lim": (0.9, 30),
    "overlap_y_scale": "linear",  # unused here
    "eig_y_scale": "linear",
    "overlap_y_lim": None,        # unused here
    "eig_y_lim": None,
    ...
    "x_ticks": None,
    "overlap_y_ticks": None,  # unused here
    "eig_y_ticks": [0, 0.1, 0.2],

    "x_num_ticks": None,
    "x_log_base": 10,
    "overlap_y_num_ticks": None,   # unused here
    "eig_y_num_ticks": None,

    "x_use_tex_log_formatter": False,

    # ---- Per-model markers/colors/LINESTYLES ----
    "per_model_style": {
    ...
    # ---- Global defaults ----
    "overlap_marker_size": 2.0,  # unused here
    "overlap_linestyle": "None", # unused here
    "overlap_linewidth": 0.5,    # unused here
...
# ---------- Build figure (1 row x 1 col): Eigenvalues ----------
fig, ax_eig = plt.subplots(
...
# Scales
ax_eig.set_yscale(PLOT_CFG["eig_y_scale"])
ax_eig.set_xscale("log" if PLOT_CFG["x_scale"] == "log" else "linear")

# Grid & spines
if PLOT_CFG["show_grid"]:
    ax_eig.grid(True, axis=PLOT_CFG["grid_axis"],
                alpha=PLOT_CFG["grid_alpha"], linestyle=PLOT_CFG["grid_linestyle"])
else:
    ax_eig.grid(False)
for spine, vis in PLOT_CFG["spines_visible"].items():
    if spine in ax_eig.spines:
        ax_eig.spines[spine].set_visible(vis)
...
for j, model_name in enumerate(model_names):
    ...
    h_eig = ax_eig.errorbar(
        xg, stats["eig_mean"][:stats["n_global"]],
        yerr=stats["eig_sem"][:stats["n_global"]],
        fmt=m, markersize=PLOT_CFG["eig_marker_size"],
        linestyle=eg_ls, linewidth=eg_lw,
        color=c,
    ...
    _raise_errorbar(
        h_eig, PLOT_CFG["error_zorder"],
        cap_lw=PLOT_CFG["error_cap_linewidth"],
        bar_lw=PLOT_CFG["error_line_linewidth"],
    )

    if PLOT_CFG["show_pr_lines"]:
        ax_eig.axvline(stats["pr_mean"], linestyle=PLOT_CFG["pr_linestyle"],
                       linewidth=PLOT_CFG["pr_linewidth"], alpha=PLOT_CFG["pr_alpha"], color=c)

    handle_eig = h_eig.lines[0] if hasattr(h_eig, "lines") and h_eig.lines else h_eig
    handles_eig.append(handle_eig)
    labels_eig.append(model_name)

# Axis labels
if PLOT_CFG["show_eig_ylabel"]:
    ax_eig.set_ylabel(PLOT_CFG["eig_ylabel"], fontsize=PLOT_CFG["ylabel_fontsize"])
if PLOT_CFG["show_xlabel"]:
    ax_eig.set_xlabel(PLOT_CFG["xlabel"], fontsize=PLOT_CFG["xlabel_fontsize"])

# Tick params
ax_eig.tick_params(axis="x", labelsize=PLOT_CFG["xtick_labelsize"], pad=PLOT_CFG["xtick_pad"])
ax_eig.tick_params(axis="y", labelsize=PLOT_CFG["ytick_labelsize"], pad=PLOT_CFG["ytick_pad"])

# Optional explicit ticks (X)
if PLOT_CFG["x_ticks"] is not None:
    ax_eig.xaxis.set_major_locator(FixedLocator(PLOT_CFG["x_ticks"]))
    ax_eig.xaxis.set_major_formatter(FixedFormatter([str(t) for t in PLOT_CFG["x_ticks"]]))
elif PLOT_CFG["x_num_ticks"] is not None and PLOT_CFG["x_scale"] == "log":
    ax_eig.xaxis.set_major_locator(LogLocator(base=PLOT_CFG["x_log_base"], numticks=PLOT_CFG["x_num_ticks"]))
    if PLOT_CFG["x_use_tex_log_formatter"]:
        ax_eig.xaxis.set_major_formatter(FuncFormatter(lambda x, pos: tex_log_formatter(x, pos, base=PLOT_CFG["x_log_base"])))

# Optional explicit ticks (Y)
if PLOT_CFG["eig_y_ticks"] is not None:
    ax_eig.yaxis.set_major_locator(FixedLocator(PLOT_CFG["eig_y_ticks"]))
    ax_eig.yaxis.set_major_formatter(FixedFormatter([str(t) for t in PLOT_CFG["eig_y_ticks"]]))
elif PLOT_CFG["eig_y_num_ticks"] is not None:
    ax_eig.yaxis.set_major_locator(MaxNLocator(PLOT_CFG["eig_y_num_ticks"]))

# Y limits (controllable)
if PLOT_CFG["eig_y_lim"] is not None:
    ax_eig.set_ylim(*PLOT_CFG["eig_y_lim"])

# Legend
eig_leg_cfg = PLOT_CFG["eig_legend"]
if eig_leg_cfg.get("show", True) and handles_eig:
    ax_eig.legend(
        handles_eig, labels_eig,
        loc=eig_leg_cfg.get("loc", "upper right"),
        ncol=eig_leg_cfg.get("ncols", 1),
        fontsize=eig_leg_cfg.get("fontsize", 8),
        frameon=eig_leg_cfg.get("frameon", False),
        handlelength=eig_leg_cfg.get("handlelength", 1.2),
        handleheight=eig_leg_cfg.get("handleheight", 0.6),
        handletextpad=eig_leg_cfg.get("handletextpad", 0.4),
        labelspacing=eig_leg_cfg.get("labelspacing", 0.2),
        borderaxespad=eig_leg_cfg.get("borderaxespad", 0.2),
        columnspacing=eig_leg_cfg.get("columnspacing", 0.8),
        markerscale=eig_leg_cfg.get("markerscale", 1.0),
    )
