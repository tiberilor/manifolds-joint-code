...
# END NEW
# NEW: category holdout selection (match training via same Q/seed)
...
# NEW: Global dimensionality reduction
...
parser.add_argument("--n_gamma_cv_splits", type=int, default=1, help="inner cv splits over which the best gamma is selected")
parser.add_argument("--gammas", type=float, nargs="+", default=None,
                    help="Optional list of global regularizer strengths. If provided, overrides --gamma.")
parser.add_argument("--gamma", type=float, default=0.2,
                    help="Global regularizer strength (single-run fallback if --gammas is not provided).")
...
# NEW: dihedral augmentation (D4) implies 8 variants per base image
...
def accumulate_cpu(old, addition):
    """
    Accumulate tensors on CPU (to save GPU RAM) — use ONLY for intermediate_results.
    """
    if isinstance(addition, torch.Tensor):
        addition_cpu = addition.detach().cpu()
    else:
        addition_cpu = copy.deepcopy(addition)
    if old is None:
        return addition_cpu.clone() if isinstance(addition_cpu, torch.Tensor) else addition_cpu
    else:
        return old + addition_cpu
...
def explained_variance_ratio(eigs: torch.Tensor, dims_kept: int) -> torch.Tensor:
    ...
    Returns:
        torch.Tensor: Scalar tensor with the explained variance ratio in [0, 1].
                      If total variance is 0, returns NaN (mirrors your function's semantics).
    """
    ...
    # Match the division behavior in your function (cumulative / total):
...
# Support a list of gammas; fall back to the single --gamma for backward compatibility
gammas = args.gammas if args.gammas is not None else [args.gamma]
print(f"Sampling gammas: {gammas}", flush=True)
...
# determine appropriate dataloader and features TODO: implement here new datasets eventually
...
if dataset_folder_name in ["SDXL_dataset_test", "SDXL_dataset_train", "SDXL_dataset_validation",
    ...
    print(f"ERROR: dataset {dataset_folder_name} is not in the implemented datasets", flush=True)
    exit(1)
...
# NEW: excluded/held-out categories (deterministic; matches training if you use same Q/seed)
excluded_categories = []
...
# NEW: manifolds subsampling (with optional macro-balanced sampling via --manifold_subsampling_grouping_json)
...
# One accumulator set per gamma. We still only keep *sums* over categories (A and W_tilde),
# so memory stays small and does not scale with #categories.
W0_dictionary_by_gamma = {
    g: {
        "W0_inner": {feature: {split: {inner: None for inner in range(args.n_gamma_cv_splits)}
                                    for split in range(args.n_cv_splits)} for feature in features},
        "W0_outer": {feature: {split: None for split in range(args.n_cv_splits)} for feature in features},
        "W0_full": {feature: None for feature in features},
    } for g in gammas
}

intermediate_results_by_gamma = {
    g: {
        "W_tilde_inner": {feature: {split: {inner: None for inner in range(args.n_gamma_cv_splits)}
                                    for split in range(args.n_cv_splits)} for feature in features},
        "A_inner": {feature: {split: {inner: None for inner in range(args.n_gamma_cv_splits)}
                              for split in range(args.n_cv_splits)} for feature in features},
        "W_tilde_outer": {feature: {split: None for split in range(args.n_cv_splits)} for feature in features},
        "A_outer": {feature: {split: None for split in range(args.n_cv_splits)} for feature in features},
        "W_tilde_full": {feature: None for feature in features},
        "A_full": {feature: None for feature in features},
    } for g in gammas
}
...
for category in categories:
    ...
    # <editor-fold desc="Define dictionaries">
    full = {
        "covariance": None,
        "mean": None,
        "n_images": None,
    ...
    inner_train = {
        "covariance": {split: {inner: None for inner in range(args.n_gamma_cv_splits)} for split in
                       range(args.n_cv_splits)},
        "mean": {split: {inner: None for inner in range(args.n_gamma_cv_splits)} for split in range(args.n_cv_splits)},
        "n_images": {split: {inner: None for inner in range(args.n_gamma_cv_splits)} for split in range(args.n_cv_splits)},
        "io_covariance": {
            feature: {split: {inner: None for inner in range(args.n_gamma_cv_splits)} for split in range(args.n_cv_splits)}
    ...
    # END NEW
        ...
        for split, (train_idx, test_idx) in enumerate(splits):
            ...
            # NOTE TO MYSELF: I know that computing the train here is computing it twice as we compute it splitted also
            # in the inner loop. But I have not enough willpower right now to implement train as derived from
            # inner_train + inner_split. We will spend a bit more computational power.
            ...
            for inner_split, (inner_train_idx, inner_test_idx) in enumerate(inner_splits):
                ...
                inner_train["n_images"][split][inner_split] = accumulate(inner_train["n_images"][split][inner_split], inner_n_images_train)

                inner_n_images_test = inner_reps_test.size(0)
                inner_test["n_images"][split][inner_split] = accumulate(inner_test["n_images"][split][inner_split], inner_n_images_test)

                # update covariance
                inner_cov_train_addition = einsum(inner_reps_train, inner_reps_train, "image neuron1, image neuron2 -> neuron1 neuron2")
                inner_train["covariance"][split][inner_split] = accumulate(inner_train["covariance"][split][inner_split], inner_cov_train_addition)

                inner_cov_test_addition = einsum(inner_reps_test, inner_reps_test, "image neuron1, image neuron2 -> neuron1 neuron2")
                inner_test["covariance"][split][inner_split] = accumulate(inner_test["covariance"][split][inner_split], inner_cov_test_addition)

                # update mean
                inner_mean_train_addition = torch.sum(inner_reps_train, dim=0)
                inner_train["mean"][split][inner_split] = accumulate(inner_train["mean"][split][inner_split], inner_mean_train_addition)

                inner_mean_test_addition = torch.sum(inner_reps_test, dim=0)
                inner_test["mean"][split][inner_split] = accumulate(inner_test["mean"][split][inner_split], inner_mean_test_addition)
    ...
    # <editor-fold desc="Fill the full dictionary">
    # Note: We just add train and test for one split
    full["covariance"] = train["covariance"][0] + test["covariance"][0]
    full["mean"] = train["mean"][0] + test["mean"][0]
    full["n_images"] = train["n_images"][0] + test["n_images"][0]
    for feature in features:
        full["label_mean"][feature] = train["label_mean"][feature][0] + test["label_mean"][feature][0]
        full["label_variance"][feature] = train["label_variance"][feature][0] + test["label_variance"][feature][0]
        full["io_covariance"][feature] = train["io_covariance"][feature][0] + test["io_covariance"][feature][0]
    # </editor-fold>

    # <editor-fold desc="Divide by total number of images">
    ...
    # <editor-fold desc="subtract means from covariances">
    ...
    # <editor-fold desc="Repeat The same as the above for the inner splits">
    # divide by total number of images
    for split in range(args.n_cv_splits):
        for inner_split in range(args.n_gamma_cv_splits):
            ...
            for feature in features:
                ...
                inner_train["label_variance"][feature][split][inner_split] /= inner_train["n_images"][split][
                    inner_split]
    ...
    for split in range(args.n_cv_splits):
        for inner_split in range(args.n_gamma_cv_splits):
            inner_train["covariance"][split][inner_split] -= torch.outer(inner_train["mean"][split][inner_split],
                                                                         inner_train["mean"][split][inner_split])
            inner_test["covariance"][split][inner_split] = (
                    inner_test["covariance"][split][inner_split]
                    ...
                    + torch.outer(inner_train["mean"][split][inner_split], inner_train["mean"][split][inner_split])
            )

            for feature in features:
                inner_train["label_variance"][feature][split][inner_split] -= inner_train["label_mean"][feature][split][
                                                                                  inner_split] ** 2
                inner_test["label_variance"][feature][split][inner_split] = (
                        inner_test["label_variance"][feature][split][inner_split]
                        - 2.0 * inner_train["label_mean"][feature][split][inner_split] *
                        inner_test["label_mean"][feature][split][inner_split]
                        + (inner_train["label_mean"][feature][split][inner_split] ** 2)
                )

                inner_train["io_covariance"][feature][split][inner_split] -= inner_train["mean"][split][inner_split] * \
                                                                             inner_train["label_mean"][feature][split][
                                                                                 inner_split]
                inner_test["io_covariance"][feature][split][inner_split] = (
                        inner_test["io_covariance"][feature][split][inner_split]
                        - inner_train["mean"][split][inner_split] * inner_test["label_mean"][feature][split][
                            inner_split]
                        - inner_test["mean"][split][inner_split] * inner_train["label_mean"][feature][split][
                            inner_split]
                        + inner_train["mean"][split][inner_split] * inner_train["label_mean"][feature][split][
                            inner_split]
                )
    # </editor-fold>

    # UPDATE GLOBAL QUANTITIES FOR DETERMINING W0
    for feature in features:
        ...
        for split in range(args.n_cv_splits):
            for inner_split in range(args.n_gamma_cv_splits):
                ...
                I = torch.eye(C_tr.size(0), device=C_tr.device, dtype=C_tr.dtype)

                # Do the (C+gI) Cholesky for *each* gamma and accumulate A_mu and W_tilde_mu into that gamma's bucket
                for g in gammas:
                    C_reg = C_tr + (g * I)
                    L = torch.linalg.cholesky(C_reg)

                    # W_tilde_mu = (C_tr + g I)^(-1) d_tr   without explicitly forming the inverse
                    W_tilde_mu = torch.cholesky_solve(d_tr.unsqueeze(1), L).squeeze(1)

                    # A_mu = (C_tr + g I)^(-1)
                    A_mu = torch.cholesky_inverse(L)

                    intermediate_results_by_gamma[g]["A_inner"][feature][split][inner_split] = accumulate_cpu(
                        intermediate_results_by_gamma[g]["A_inner"][feature][split][inner_split], A_mu)
                    intermediate_results_by_gamma[g]["W_tilde_inner"][feature][split][inner_split] = accumulate_cpu(
                        intermediate_results_by_gamma[g]["W_tilde_inner"][feature][split][inner_split], W_tilde_mu)

            # ----- OUTER SPLIT (same idea) -----
            ...
            for g in gammas:
                ...
                intermediate_results_by_gamma[g]["A_outer"][feature][split] = accumulate_cpu(
                    intermediate_results_by_gamma[g]["A_outer"][feature][split], A_mu)
                intermediate_results_by_gamma[g]["W_tilde_outer"][feature][split] = accumulate_cpu(
                    intermediate_results_by_gamma[g]["W_tilde_outer"][feature][split], W_tilde_mu)

        # ----- FULL DATA (same idea) -----
        print("\t\t\tPROCESSING FULL")
        C_tr = full["covariance"]
        d_tr = full["io_covariance"][feature]
        I = torch.eye(C_tr.size(0), device=C_tr.device, dtype=C_tr.dtype)
        ...
        for g in gammas:
            ...
            intermediate_results_by_gamma[g]["A_full"][feature] = accumulate_cpu(
                intermediate_results_by_gamma[g]["A_full"][feature], A_mu)
            intermediate_results_by_gamma[g]["W_tilde_full"][feature] = accumulate_cpu(
                intermediate_results_by_gamma[g]["W_tilde_full"][feature], W_tilde_mu)

# ===== HARD FREE: drop big dicts & transient tensors we won't use anymore =====
import gc

# 2) Drop batch / rep buffers if they exist
for _name in [
    "images", "gt_bboxes",
    "reps", "reps_h",
    "reps_train", "reps_test",
    "inner_reps_train", "inner_reps_test",
    "batch", "batch_inputs", "batch_outputs",
]:
    if _name in locals():
        del locals()[_name]

# 3) Drop large stats dicts we won't use anymore (W0 uses only intermediate_results)
for _name in ["train", "test", "inner_train", "inner_test", "full"]:
    if _name in locals():
        del locals()[_name]
...
gc.collect()
print("[mem] Deleted train/test/inner/full dicts and transient tensors; keeping model and intermediate_results.", flush=True)


# DIVIDE GLOBAL INTERMEDIATE RESULTS BY TOTAL NUMBER OF CATEGORIES
for g in gammas:
    for feature in features:
        intermediate_results_by_gamma[g]["A_full"][feature] /= n_categories
        intermediate_results_by_gamma[g]["W_tilde_full"][feature] /= n_categories
        for split in range(args.n_cv_splits):
            intermediate_results_by_gamma[g]["A_outer"][feature][split] /= n_categories
            intermediate_results_by_gamma[g]["W_tilde_outer"][feature][split] /= n_categories
            for inner_split in range(args.n_gamma_cv_splits):
                intermediate_results_by_gamma[g]["A_inner"][feature][split][inner_split] /= n_categories
                intermediate_results_by_gamma[g]["W_tilde_inner"][feature][split][inner_split] /= n_categories


# DETERMINE GLOBAL COMMON MEAN VECTOR for each gamma
for g in gammas:
    for feature in features:
        for split in range(args.n_cv_splits):
            for inner_split in range(args.n_gamma_cv_splits):
                A = intermediate_results_by_gamma[g]["A_inner"][feature][split][inner_split].to(device)
                W_tilde = intermediate_results_by_gamma[g]["W_tilde_inner"][feature][split][inner_split].to(device)
                I = torch.eye(A.size(0), device=A.device, dtype=A.dtype)
                # Solve (I - g A) W0 = W_tilde
                W0 = torch.linalg.solve(I - (g * A), W_tilde)
                W0_dictionary_by_gamma[g]["W0_inner"][feature][split][inner_split] = W0.detach().cpu().clone()

            # OUTER
            A = intermediate_results_by_gamma[g]["A_outer"][feature][split].to(device)
            W_tilde = intermediate_results_by_gamma[g]["W_tilde_outer"][feature][split].to(device)
            I = torch.eye(A.size(0), device=A.device, dtype=A.dtype)
            W0 = torch.linalg.solve(I - (g * A), W_tilde)
            W0_dictionary_by_gamma[g]["W0_outer"][feature][split] = W0.detach().cpu().clone()

        # FULL
        A = intermediate_results_by_gamma[g]["A_full"][feature].to(device)
        W_tilde = intermediate_results_by_gamma[g]["W_tilde_full"][feature].to(device)
        I = torch.eye(A.size(0), device=A.device, dtype=A.dtype)
        W0 = torch.linalg.solve(I - (g * A), W_tilde)
        W0_dictionary_by_gamma[g]["W0_full"][feature] = W0.detach().cpu().clone()

# SECOND LOOP
print("Performing the second loop")

# We will emit one results file per gamma to stay 100% compatible downstream.
args_dictionary = vars(args).copy()
final_results_by_gamma = {
    g: {
        "args": {**args_dictionary, "gamma": g},  # overwrite 'gamma' field so filenames/metadata match old runs
        "local": {},
        # "global": W0_dictionary_by_gamma[g],  # intentionally omitted to avoid storing W0_* in output files
    } for g in gammas
}
...
for category in categories:
    ...
    # For each gamma, build and store the same per-category 'results' dict your pipeline expects
    for g in gammas:
        print(f"[category={category}] Computing per-split regressions for gamma={g}", flush=True)

        # define the local results dictionary (same shape/keys as before)
        results = {
            "inner_mse_test": {feature: {split: {inner_split: None for inner_split in range(args.n_gamma_cv_splits)}
                                         for split in range(args.n_cv_splits)} for feature in features},
            "inner_normalized_mse_test": {
                feature: {split: {inner_split: None for inner_split in range(args.n_gamma_cv_splits)}
                          for split in range(args.n_cv_splits)} for feature in features},
            "inner_labels_variance": {
                feature: {split: {inner_split: None for inner_split in range(args.n_gamma_cv_splits)}
                          for split in range(args.n_cv_splits)} for feature in features},
            "inner_mse_test_avg": {feature: {split: None
                                             for split in range(args.n_cv_splits)} for feature in features},
            "inner_mse_test_std": {feature: {split: None
                                             for split in range(args.n_cv_splits)} for feature in features},
            "inner_normalized_mse_test_avg": {feature: {split: None
                                                        for split in range(args.n_cv_splits)} for feature in features},
            "inner_normalized_mse_test_std": {feature: {split: None
                                                        for split in range(args.n_cv_splits)} for feature in features},
            "mse_test": {feature: {split: None for split in range(args.n_cv_splits)} for feature in features},
            "normalized_mse_test": {feature: {split: None for split in range(args.n_cv_splits)} for feature in
                                    features},
            "labels_variance": {feature: {split: None for split in range(args.n_cv_splits)} for feature in features},
            ...
            "regression_vector": {feature: None for feature in features},
        }

        # DO THE CV REGRESSION (identical math, but use gamma=g and the gamma-specific W0 we precomputed)
        for feature in features:
            print(f"Feature: {feature}", flush=True)
            outer_mses = []
            ...
            for split in range(args.n_cv_splits):
                print(f"\tOuter split: {split}", flush=True)
                inner_mses = []
                ...
                for inner_split in range(args.n_gamma_cv_splits):
                    ...
                    W0 = W0_dictionary_by_gamma[g]["W0_inner"][feature][split][inner_split].to(C_tr.device)

                    I = torch.eye(C_tr.size(0), device=C_tr.device, dtype=C_tr.dtype)
                    w = torch.linalg.solve(C_tr + g * I, d_tr + g * W0)
                    ...
                    in_mse_normalized = in_mse / actual_variance

                    inner_mses.append(in_mse.item())
                    inner_mses_normalized.append(in_mse_normalized.item())
                    results["inner_mse_test"][feature][split][inner_split] = in_mse
                    results["inner_normalized_mse_test"][feature][split][inner_split] = in_mse_normalized
                    results["inner_labels_variance"][feature][split][inner_split] = actual_variance

                # summarize inner
                results["inner_mse_test_avg"][feature][split] = np.mean(inner_mses)
                results["inner_mse_test_std"][feature][split] = np.std(inner_mses)
                results["inner_normalized_mse_test_avg"][feature][split] = np.mean(inner_mses_normalized)
                results["inner_normalized_mse_test_std"][feature][split] = np.std(inner_mses_normalized)
                print(
                    f"\t\t\tgamma: {g}, avg inner mse: {results['inner_mse_test_avg'][feature][split]} +- {results['inner_mse_test_std'][feature][split]}, "
                    f"nmse: {results['inner_normalized_mse_test_avg'][feature][split]} +- {results['inner_normalized_mse_test_std'][feature][split]}",
                    flush=True)

                # OUTER LOOP
                C_tr = train["covariance"][split]
                d_tr = train["io_covariance"][feature][split]
                W0 = W0_dictionary_by_gamma[g]["W0_outer"][feature][split].to(C_tr.device)
                ...
                outer_mse = (w @ C_te @ w) - 2 * (w @ d_te) + y2
                ...
                outer_normalized_mse = outer_mse / actual_variance

                results["labels_variance"][feature][split] = actual_variance
                ...
                print(f"\t\tgamma: {g}, outer mse: {outer_mse}, nmse: {outer_normalized_mse}", flush=True)

            # summary over outer splits
            results["labels_variance_full"][feature] = full["label_variance"][feature]
            results["mse_test_avg"][feature] = np.mean(outer_mses)
            results["mse_test_std"][feature] = np.std(outer_mses)
            results["normalized_mse_test_avg"][feature] = np.mean(outer_normalized_mses)
            results["normalized_mse_test_std"][feature] = np.std(outer_normalized_mses)

            print(
                f"\tgamma: {g}, avg outer mse: {results['mse_test_avg'][feature]} +- {results['mse_test_std'][feature]}, "
                f"nmse: {results['normalized_mse_test_avg'][feature]} +- {results['normalized_mse_test_std'][feature]}",
                flush=True)
            print(
                f"\tgamma: {g}, avg outer mse / full labels variance: {results['mse_test_avg'][feature] / results['labels_variance_full'][feature].item()} "
                f"+- {results['mse_test_std'][feature] / results['labels_variance_full'][feature].item()}", flush=True)

            # FULL-DATA REGRESSION VECTOR
            C_full = full["covariance"]
            d_full = full["io_covariance"][feature]
            W0_full = W0_dictionary_by_gamma[g]["W0_full"][feature].to(C_full.device)
            I = torch.eye(C_full.size(0), device=C_full.device, dtype=C_full.dtype)
            w_full = torch.linalg.solve(C_full + g * I, d_full + g * W0_full)
            results["regression_vector"][feature] = w_full

        # store per-gamma per-category results
        results_numpy = convert_tensors_to_numpy(results)
        final_results_by_gamma[g]["local"][category] = results_numpy
...
# NEW: added manifolds subsampling string
for g in gammas:
    core_name = (f"PipelineV3_CVregressionGammaOnly_{dataset_name}_{args.layer}"
                 f"_nOutSplit{args.n_cv_splits}"
                 f"_nInSplit{args.n_gamma_cv_splits}"
                 f"{rnd_proj_string}"
                 f"{rnd_sub_string}"
                 f"{manifolds_sub_string}"
                 f"{global_dim_red_string}"
                 f"_gamma{g}")
    results_file = os.path.join(results_dir, f"{args.job_id}_{core_name}.pkl")
    with open(results_file, 'wb') as file:
        pickle.dump(final_results_by_gamma[g], file)
