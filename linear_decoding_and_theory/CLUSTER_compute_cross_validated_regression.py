...
# NEW: Random neuron subsampling (analogous to random projection)
...
# TODO: add these new options to the GPU SRUN script
parser.add_argument(
    "--outer_split_method",
    type=str,
...
# NEW: global subsample index (reused across all batches/categories)
subsample_idx = None
# END NEW
...
for category in categories:
    ...
    for batch in dataloader:
        ...
        # NEW: define the random subsample indices if required (once)
...
# NEW: filename tag for subsampling (analogous to random projection)
if args.random_subsample_size is not None:
...
# NEW: added rnd_sub_string to results file name
results_file = os.path.join(results_dir, f"{args.job_id}_CVregression_{dataset_name}_{args.layer}_nOutSplit{args.n_cv_splits}_nInSplit{args.n_l2_cv_splits}" + rnd_sub_string + rnd_proj_string + global_dim_red_string + manifolds_sub_string + ".pkl")
# END NEW
