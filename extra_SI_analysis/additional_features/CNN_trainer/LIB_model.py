...
class ResNetBBoxModel(nn.Module):
    _CANONICAL_BBOX_FEATURES = ["bbox_center_x", "bbox_center_y", "bbox_x_length", "bbox_y_length"]

    def __init__(self, resnet_version="resnet50", num_classes=265,
                 bbox_heads=None, class_head=True,
                 input_resolution=(224, 224), per_class_bbox: bool = False,
                 per_class_regression=None, regression_head_features=None):
        super().__init__()

        if bbox_heads is None:
            bbox_heads = [True, True, True, True]
        bbox_heads = list(bbox_heads)

        if per_class_regression is None:
            per_class_regression = per_class_bbox
        self.per_class_regression = bool(per_class_regression)
        self.per_class_bbox = self.per_class_regression  # old checkpoint/script compatibility

        if regression_head_features is None:
            regression_head_features = self._CANONICAL_BBOX_FEATURES[:len(bbox_heads)]
            if len(bbox_heads) > len(regression_head_features):
                regression_head_features += [
                    f"regression_{i}" for i in range(len(regression_head_features), len(bbox_heads))
                ]
        self.regression_head_features = list(regression_head_features)
        if len(self.regression_head_features) != len(bbox_heads):
            raise ValueError(
                "regression_head_features must have the same length as bbox_heads. "
                f"Got {len(self.regression_head_features)} features and {len(bbox_heads)} heads."
            )
        ...
        self.bbox_heads = nn.ModuleList([
            (nn.Linear(self.backbone_output_dim, num_classes) if self.per_class_regression
        ...
        for name, child in self.backbone.named_children():
            print(name)
        print(f"Regression heads: {self.regression_head_features}")
        print(f"Per-class regression: {self.per_class_regression}")
